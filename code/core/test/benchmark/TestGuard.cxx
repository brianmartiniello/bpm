...
struct Data { int value; bool valid; };
void process(int in, int& out) { out = in * 2; }

// --- 1. Manual Approach ---
static void BM_Manual(benchmark::State& state) {
    std::vector<Data> input(state.range(0), {10, true});
    ...
    for (auto _ : state) {
        for (size_t i = 0; i < input.size(); ++i) {
            ...
            process(input[i].value, output[i].value);
            output[i].valid = true;
        }
...
struct DedicatedGuard {
    Data& item; bool committed = false;
    explicit DedicatedGuard(Data& i) : item(i) { item.valid = false; }
    void commit() { item.valid = true; committed = true; }
    ~DedicatedGuard() { if (!committed) item.valid = false; }
};
...
template <typename F>
struct GenericGuard {
    F cleanup; bool active = true;
    ~GenericGuard() { if (active) cleanup(); }
